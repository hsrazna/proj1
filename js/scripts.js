...
//, .form-container form
	$(".red-form, .modal-c form").on("submit",function(e){
		...
		var t=$(this),
		n=$.trim($(".current-form input[name=phone]").val()),
		a=$.trim($(".current-form input[name=email]").val()),
		r=$(".current-form input[name=phone]"),
		o=($(".current-form input[name=email]"),$(this).serializeArray());
		var form_data = $(this).serialize();
		null!=n&&0==n.length?
		(alert("Укажите телефон"),$(r).addClass("input-error"),event.preventDefault()):
		null!=a&&0==a.length?
		(alert("Укажите почту"),$(r).addClass("input-error"),event.preventDefault()):
		(
